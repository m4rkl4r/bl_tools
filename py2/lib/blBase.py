...
class blBase:
    ...
    serverGroup=None
    serverMap=None
    serverMapCsv=[local fs location of server map]
    UIfile=[local file with json mapping of app servers]
    UIbackends=None
    suds_lock=None
    loginattempts=0
    ipdbfile=None
    ...
    bladelogicticketurl=[url for creating web ticket]
    ...
    def getMyGroup(): 
        ...
        #conf.read(cfg)
        #return conf.get('Bladelogic', 'myGroup')
        return os.environ.get('myGroup')
    ...
    @staticmethod
    def getMyConf():
        if os.environ.has_key("BL_CONF"):
            return os.environ.get("BL_CONF")
        else:
    ...
    def initServerMap(self):
        ...
        servermap = open(self.serverMapCsv, "r")
    ...
    def getMyHome():
        ...
        elif os.environ.has_key("PBUSER"):
            user=os.environ.get("PBUSER")
        else:
...
#            sys.exit(1)
    ...
    def mkdir_recursive(self,grouptype,completed,remaining):
        ...
        #            print "creating",grouptype,completed + ":",
                    ...
                    print "self.runBlcli(",grouptype,"createGroupWithParentName",[childGroup,parentGroup],
...
#
    ...
    def depotObjectDBKey(self,groupName,fileName):
        for filetype in blLib.blDepotObjectTypeShort.keys():
    ...
    def jobGroupDBKey(self,groupName):
        print "JobGroup",groupName,": trying to find dbkey:",
    ...
    def jobDBKey(self,jobGroup,jobName):
        ...
        for jobtype in blLib.blGetDBKeyJobTypes:
            ...
            print jobGroup,jobName,": trying",jobtype,"to find dbkey:",
    ...
    def validateServerList(self,serverlist,verbose=False): # returns a dict with lists of "good" and "bad" servers, good mapped to fqdn used by bladelogic
        goodservers=list()
        badservers=list()
        servermap=dict()
        ...
        for server in serverlist:
            if verbose:
                print "found",server,"as",
            server = server.split(".")[0].lower()
            fqdn = self.blFQDN(server)
            ...
            if fqdn:
                ...
                #if self.runBlcli("Server","getServerIdByName",[fqdn]).success:
                #    if verbose: print fqdn,"in BL:",True
                #    goodservers.append(fqdn)
                else:   
                    if verbose: print fqdn,"in BL:",False
                    badservers.append(fqdn)
            else:
                if verbose: print None,"in BL:",False
...
#        else: return result
    ...
    def localBlFQDN(self):
        ...
        #print "hostname",hostname
        fqdn = self.blFQDN(hostname)
        #print "fqdn",fqdn
        #print "done localBlFQDN"
        return fqdn


    def blFQDN(self, hostname):
        server = hostname.split(".")[0].lower()
        fqdn = None
        if self.serverMap.has_key(server):
            fqdn = self.serverMap[server]
        else: 
            print "blFQDN: " + hostname + " fqdn not found in serverMap"
        return fqdn

    def inventoryFQDN(self,hostname,usedb=True):
	# whatever you need to do to get fqdn from our inventory system
    ...
    def initLogging(self):
        ...
        logging.basicConfig(level=logging.WARN,format='%(asctime)s %(name) %(LeveLname)s %(message)s',filename=filename)
    ...
    def emailError(self,url,e):
        ...
        if not os.environ["USER"] == [THE TESTING USER]: return

        from subprocess import Popen
        process = Popen(["ssh","mailerhost","mail","-vv","-s","bl_tools.failed_login", "--", "email@address.com"],stdin=PIPE, stdout=PIPE)
        process.stdin.write(text)
        print process.communicate()[0]
        process.stdin.close()
        
    def initClients(self,retry=False,failquick=False):
        print("login attempt "+str(self.trynum))
        ...
        print >> sys.stderr, 'Logged in as: '+self.blUser
        print >> sys.stderr, 'Session ID: ' + str(self.sessionId)
        print >> sys.stderr, 'Switching role to: '+ self.blRole
    ...
    def initAlternateURLs(self):
        ...
        file = open(self.UIfile,"r")
        ALL_UI=json.load(file)
        vip = [the front line appserver fqdn]
        vip_addr = socket.gethostbyname(vip)
        vip1 = [1st regional appserver fqdn]
        vip1_addr = socket.gethostbyname(vip1)
        vip2 = [2nd regional appserver fqdn]
        vip2_addr = socket.gethostbyname(vip2)

        if vip_addr == vip1_addr: POD="POD1"
        elif vip_addr == vip2_addr: POD="POD2"
        else:
            print "ERROR: [front line appserver fqdn] (",vip_addr,")is not pointing at [first regional appserver fqdn] (",vip1_addr,") or [2nd regional appserver fqdn] (",vip2_addr,")"
            sys.exit(1)
    ...
    def __init__(self,debug=False,bl_conf=None,failquick=False):
        ...
        if not os.environ.has_key("myRole"):
...
ns = { 'status': 'http://[enrollment check webservice url]' }

def blStatus(fqdn):
    url=[url to web service] + fqdn
...
def blPerms(fqdn):
    url = [url to webservice] + fqdn
    headers = {'Content-Type': 'text/xml'}
    content = requests.get(url,headers=headers,verify=False).content
    return re.search("Permissions>(.*)<\/Permissions", content).group(1).split(" ")

def blSAMs(fqdn): return filter(lambda x: re.search("SAM_",x), blPerms(fqdn))
def blRoles(fqdn): return filter(lambda x: not re.search("SAM_",x), blPerms(fqdn))
def blCustomRoles(fqdn): return filter(lambda x: re.search("CUSTOM",x) and re.search("UNIX",x), blRoles(fqdn))
